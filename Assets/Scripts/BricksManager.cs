...
public class BricksManager : MonoBehaviour
{
   ...
   private int maxRows = 17; //метод максимум строк
   private int maxCols = 12; //метод максимум колонок
   ...
   {
       TextAsset text = Resources.Load("levels") as TextAsset; //подключение к загрузке внешнего ресурса с комбинацией уровней

       string[] rows = text.text.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);

       List<int[,]> levelsData = new List<int[,]>();
       int[,] currentLevel = new int[maxRows, maxCols]; //опции методов о строках и колонках
       int currentRow = 0;
